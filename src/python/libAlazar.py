...
class LibAlazar():
    ...
    def __init__(self):
        ...
        self._setAll.argtypes = [c_uint32, POINTER(self.ConfigData),
                                 POINTER(self.AcquisitionParams)]
        ...
        self._wait_for_acquisition.argtypes = [c_uint32, POINTER(c_float),
                                               POINTER(c_float)]
    ...
    def setAll(self, config):
        ...
        retVal = self._setAll(self.boardId, byref(self.configData),
                              byref(self.acquisitionParams))
        if retVal < 0:
            return retVal
    ...
    def wait_for_acquisition(self):
        retVal = self._wait_for_acquisition(self.boardId, self.ch1Buffer_p,
                                            self.ch2Buffer_p)
        return retVal
    ...
    def generateTestPattern(self):
        ...
        ch1 = np.zeros((self.config['recordLength'], numRecords),
                       dtype=np.float32)
        ch2 = np.zeros((self.config['recordLength'], numRecords),
                       dtype=np.float32)
        for r in range(numRecords):
            #apply the scaling and offset - recreate the pattern from alazarSim.c
            ch1[:, r] = (
                np.mod(r, 256) - 128.) * self.config['verticalScale'] / 128.
            ch2[:, r] = (np.mod(r + 1, 256) - 128.
                         ) * self.config['verticalScale'] / 128.

            #average the data if ebabled
        if self.config['acquireMode'] == 'averager':
            newShape = (
                self.config['recordLength'], self.config['nbrWaveforms'],
                self.config['nbrSegments'], self.config['nbrRoundRobins'])
...
def main():
    ...
    parser.add_argument('--log',
                        help='log file none - defaults to stdout',
                        default=None)
    parser.add_argument('--acquireMode',
                        help='digitizer or averager',
                        default='averager')
    ...
    parser.add_argument('--delay',
                        ...
                        type=float,
                        default=0.0)
    parser.add_argument('--recordLength',
                        help='record length',
                        type=int,
                        default=4096)
    parser.add_argument('--nbrSegments',
                        help='number of segments',
                        type=int,
                        default=1)
    parser.add_argument('--nbrWaveforms',
                        help='number of waveforms',
                        type=int,
                        default=1)
    parser.add_argument('--nbrRoundRobins',
                        help='number of round robins',
                        type=int,
                        default=1)
    parser.add_argument('--samplingRate',
                        ...
                        type=float,
                        default=500e6)
    parser.add_argument('--triggerCoupling',
                        help='trigger coupling AC or DC',
                        default='AC')
    parser.add_argument('--triggerLevel',
                        help='trigger level in mV',
                        type=int,
                        default=1000)
    parser.add_argument('--triggerSlope',
                        help='rising or falling',
                        default='rising')
    parser.add_argument('--triggerSource', help='A, B, or Ext', default='Ext')
    parser.add_argument('--verticalCoupling',
                        help='channel coupling AC or DC',
                        default='AC')
    parser.add_argument('--verticalOffset',
                        help='channel offset in volts',
                        type=float,
                        default=0.0)
    parser.add_argument(
        '--verticalScale',
        help='channel input range, choose from [.04,.1,.2,.4,1.0,2.0,4.0]',
        type=float,
        default=1.0)
    parser.add_argument('--bufferSize',
                        help='defaults to 81920',
                        type=int,
                        default=81920)
